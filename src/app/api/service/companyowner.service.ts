...
export class CompanyOwnerService {
    ...
    async deleteCompany(companyId: string): Promise<void> {
        const company = await Company.findOne({ select: ['id'], where: { id: companyId } });
    ...
    async createCompany(dto: CreateCompanyDtoType): Promise<Omit<Company, 'password' | 'refreshToken'>> {
        const existingCompany = await Company.findOne({ where: { email: dto.email } });
        if (existingCompany) {
    ...
    async getAllJobsWithFormsAndTests(companyId?: string): Promise<any[]> {
        ...
        const applicationCounts = await Application.createQueryBuilder('application')
